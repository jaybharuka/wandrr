...
export default function ChatModal({ myUserId, myName, connection, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#1e1e1e] w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl flex flex-col shadow-2xl dark:shadow-none border border-gray-200 dark:border-[#2a2a2a]"
        style={{ height: "min(90vh, 620px)", maxHeight: "90dvh", fontFamily: "'Inter', system-ui, sans-serif" }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-[#2a2a2a] shrink-0">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 ${otherColor}`}>
            {otherInitials}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-gray-900 dark:text-[#f5f5f5] font-semibold text-sm truncate">{connection.connection_name}</p>
            {connection.travelling_from && connection.travelling_to && (
              <p className="text-gray-400 dark:text-[#737373] text-xs truncate flex items-center gap-1">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 dark:text-[#737373] hover:text-gray-900 dark:hover:text-[#f5f5f5] text-xl leading-none shrink-0 ml-2"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {loading && (
            <div className="flex justify-center pt-10">
              <div className="animate-spin rounded-full h-6 w-6 border-2 border-gray-300 dark:border-[#333333] border-t-gray-700 dark:border-t-[#d4d4d4]"></div>
            </div>
          )}
          ...
          {!loading && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-gray-700 dark:text-[#d4d4d4] font-medium text-sm">{connection.connection_name}</p>
              <p className="text-gray-400 dark:text-[#737373] text-xs mt-1">Send a message to start the conversation!</p>
            </div>
          )}
            ...
            return (
              <div key={msg.id ?? i} className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className={`max-w-[72%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-0.5`}>
                  <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words ${isMe ? "text-white rounded-br-sm" : "bg-gray-100 dark:bg-[#2a2a2a] text-gray-900 dark:text-[#f5f5f5] rounded-bl-sm"}`}
                    style={isMe ? { backgroundColor: '#FF6B35' } : {}}>
                    {msg.content}
                  </div>
                  <p className="text-gray-400 dark:text-[#737373] text-[10px] px-1">{time}</p>
                </div>
              </div>
            );
          })}
        ...
        {/* Input */}
        <div className="px-4 py-3 border-t border-gray-100 dark:border-[#2a2a2a] shrink-0 flex items-end gap-2">
          ...
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none bg-gray-100 dark:bg-[#2a2a2a] rounded-xl px-4 py-2.5 text-base text-gray-900 dark:text-[#f5f5f5] placeholder-gray-400 dark:placeholder-[#737373] focus:outline-none focus:ring-2 focus:ring-gray-300 dark:focus:ring-[#333333] transition-colors"
